...
// Helper for smooth scrolling to next step
function scrollToNext(elementSelector) {
  setTimeout(() => {
    const el = document.getElementById(elementSelector) || document.querySelector(elementSelector);
    if (el) {
...
DOM.serviceCards.forEach(card => {
  card.addEventListener('click', () => {
    ...
    // Auto-scroll to next step based on logic
    const data = serviceData[quoteState.service];
    if (data.requiresSize) {
      scrollToNext('step-2');
    } else if (data.availableAddons.length > 0) {
      scrollToNext('step-3');
    } else {
      scrollToNext('.quote-summary-panel');
    }
  });
});
